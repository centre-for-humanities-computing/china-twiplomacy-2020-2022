...
def main(n_labels, infile, outfolder): 
    ...
    network_not_mentionee = list(set(network_handles) - set(concat_mentionee)) # in network, but not mentionee (i.e. there as mentioner)
    print(f"mentionee --> not network: {mentionee_not_network}")
    print(f"network --> not mentionee: {network_not_mentionee}")
    ...
    write_txt_file(mentionee_not_network, outfolder, "mentionee_not_network")
    write_txt_file(network_not_mentionee, outfolder, "network_not_mentionee")
    ...
    ### handle cases of no mentions for a handle ###
    ...
    nudge_triple = [
        ...
        ('CGTNOfficial', -0.1, 0.1),
        ('CGMeifangZhang', -0.05, 0.05),
        ('CHN_UN_NY', 0, 0.05),
        ('xuejianosaka', -0.05, -0.05),
        ('ConsulateSan', 0.05, -0.05),
        ('ChinaCGCalgary', 0, -0.08),
        ('ChinaMissionGva', 0, -0.1),
        ('shen_shiwei', 0, -0.05),
        ('AmbassadeChine', 0, -0.01),
        ('chenweihua', 0, 0.01)
    ]
